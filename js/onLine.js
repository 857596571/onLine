(function($){
	$.fn.onLine = function(options){
		...
		var getMoveTarget = function(pageX, pageY) {
			var doms = $('.goal').children();
			var target = null;
			doms.map(function(index, dom) {
				var domRect = dom.getBoundingClientRect();
				var xRangeMax = domRect.left + domRect.width;
				var xRangeMin = domRect.left;
				var yRangeMax = domRect.top + domRect.height;
				var yRangeMin = domRect.top;
				if (pageX > xRangeMin && pageX < xRangeMax && pageY > yRangeMin && pageY < yRangeMax) {
					target = dom
				}
			});
			return target
		}
		var mousemove = function(event){		//移动				
			var $target = $(event.target);		
			if (event.type == 'touchmove') {
				$target = $(getMoveTarget(event.changedTouches[0].pageX, event.changedTouches[0].pageY))
			}
			if(groupstate){
				if (!event.pageX) {
					var page = event.targetTouches ? event.targetTouches[0] : e.originalEvent.targetTouches[0];
					event.pageX = page.pageX;
					event.pageY = page.pageY;
				}
			...
			event.type == 'touchmove' || event.type == 'touchup' ? event.stopPropagation() : event.preventDefault();
		}
		var mouseup = function(event){  //抬起
			var $target = $(event.target);
		...
		//addstyle showitem
		box.children(".show").children().children("span").on("mousedown", mosedown);
		box.children(".show").children().children("span").on("touchstart", mosedown);
